...
export default function FavoriteContent() {
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            if (currentFavoriteElement) {
              ...
              gsap.context(() => {
                gsap.from('#audio-list', { duration: 2, opacity: 0, y: 100 })
                gsap.from('#audio-thumb', { duration: 2, opacity: 0, scale: 0 })
                gsap.from('.play-btn', { duration: 2, delay: 2, opacity: 0, scale: 0 })
                gsap.from('#listen-btn', { duration: 2, delay: 1, opacity: 0, scale: 0 })
              }, favorite)
  ...
  return (
    <div id='fav-section' className='container px-6 py-10 mx-auto bg-gray-100 relative overflow-hidden' ref={favorite}>
